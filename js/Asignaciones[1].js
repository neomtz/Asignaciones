...
{
  if($_get("source1") == null || $_get("source2") == null)
  {
    waitingDialog.show('No puedes acceder a este apartado ...', {dialogSize: 'sm', progressType: 'default'});
    return false;
  }
  else
  {
...
$('.fc-corner-left').click(function(event)
{
 ...
 if (a==1)
 {
  ...
  $("#a").css('background-color',brandEmpresa);
 ...
 if (a==2)
 {
  ...
  $("#b").css('background-color', brandEmpresa);
  $("#c").css('background-color', '#C9C9C9');
...
function getBrand()
{
		$.ajax(
	  {
	   ...
	   url: "./GET/?source=asign&source2=getbrand",
		 data: { 'source': $_get("source1") }, // url hacia el controlado
	   ...
	   {
       if(server.idUsuario == null)
       {
         waitingDialog.changeMessage("No tienes una sesión en Jarboss inciada ... o intentas accedar con un usuario distinto ...");
         return false;
       }
       ...
       brandEmpresa = server.brand;
       getChecklists(server.correoUsuario);
	   },
	   error: function(e)
	   {
		   //console.log(e);
...
function getUsuarios()
{
		$.ajax(
	  {
	   ...
	   url: "https://www.jarboss.com/Usuarios/GET/?source=lista&source2=getall",
		 data: { token: $_get("source1") }, // url hacia el controlado
		  beforeSend: function(event)
		 {
		   $("#site_active_indicador").text('1');
	   },
...
function getChecklists(correo)
{
			$.ajax(
			{
			 type: "POST", // aqui defino el canal de envio de datos GET o POST
			 dataType: "JSON", // aqui determino el tipo de dato en el que me debe responder el servidor, puede ser html /text / xml
			 url: "https://jarboss.com/API/GET/?source1=jawk&source2=checklist",
			 data:{ "source": $_get("source1"), "source1": correo, "source2": "1"},// url hacia el controlado
				beforeSend: function(event)
			 {
				 $("#site_active_indicador").text('1');
			 },
			 ...
			 $("#site_active_indicador").text('0');
       getGrupos();
			  //finaliza la petición y encadeno la siguiente esta es una buena practica.
			 },
			 error: function(e)
			 {
				 //console.log(e);
			...
			$.ajax(
				{
				 type: "POST", // aqui defino el canal de envio de datos GET o POST
				 dataType: "JSON", // aqui determino el tipo de dato en el que me debe responder el servidor, puede ser html /text / xml
				 url: "https://jarboss.com/API/GET/?source1=jawk&source2=usuario",
				 data:{ 'source': $_get("source1"),'source1': $_get("source2")},// url hacia el controlado
					beforeSend: function(event)
				 {
					 $("#site_active_indicador").text('1');
				 },
				 ...
				 {
           var grupos = [];
					 $.each(server.grupos_internos,function(a,b)
					 {
						 var lista='<option data-id="'+b.id+'" value="'+b.nombre+'"></option>';
						 $('#listaGrupos').append(lista);
             grupos.push(b.id);
					 });
           $.each(server.sucursales,function(a,b)
           {
             var lista='<option data-id="'+b.id+'" value="'+b.nombre+'"></option>';
             $('#listaSucursal').append(lista);
           });
				 $("#site_active_indicador").text('0');
				  getAsignaciones(grupos);
				 },
				 error: function(e)
				 {
					 //console.log(e);
...
function getAsignaciones(idGrupos)
{
			...
			$.ajax(
				{
				 ...
				 data:{ 'source': $_get("source1"), 'source1': '', 'source2':  $("#fechaBusqueda").text(), 'source3':idGrupos },// url hacia el controlado
					beforeSend: function(event)
				 {
...
function setAsignacion(i,tipo)
{
 ...
 var dataPost={source1:idchecklist, source2:idgrupo, source3:idsucursal, source4:idusuario, source5:fecha, source6:'', source7:'4', source: $_get("source1"),'sourcejson': json }
 if(tipo == 2)
 {
  ...
  dataPost={ 'source1':idchecklist, 'source2':idgrupo, 'source3':idusuario, 'source4':idsucursal, 'source5':fecha, 'source6':'', 'source7':'4', 'source': $_get("source1"), 'sourcejson': json}
 }
  $.ajax(
  {
   type: "POST",
...
function editarAsignacion(id)
{
   ...
   $("#formEditar").attr('src','https://www.jarboss.com/Asignaciones/Editar.html?source='+$_get("source1")+'&source1='+id+'&source2=externo')
}
