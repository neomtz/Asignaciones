...
function interfaceIncumplimiento(id,nombre,idRow,fecha,idGrupo,idUsuario,idAsignacion,fechaAsignacion,resolucion,idChecklist,idSucursal,nombreUsuario,nombreGrupo,estatus,actividad)
{
  ...
  // console.log("terminado",idRow);
    ...
    minutos = fecha2.diff(fecha1, 'minutes')
    // tiempo = dias + horas + minutos
    totalTiempo = parseInt(dias) * 24
    horas = parseInt(horas) - parseInt(totalTiempo)
    totalTiempo = parseInt(minutos) / 60
    totalTiempo = totalTiempo.toFixed(2);
    totalTiempo = totalTiempo.split(".")
    minutos = totalTiempo[1]
    if (minutos >= 60) {
      minutos = 0
    }
    tiempo= dias+ ' dias con '+horas+ ' horas con '+minutos+ ' minutos';
...
//Aqui termina modulo de volver asignar tickets
 var interfaz ='<tr class="incumplimientos '+idRow+'" style="'+oculto+'" id="'+id+'" registro="'+idRow+'" agrupados=""><td class="col10 '+exportar+'" style="vertical-align: middle;"><a href="https://www.jarboss.com/'+sessionStorage.getItem("nombreEmpresa")+'/reporteTemporal.php?source='+id+'" target="_blank">'+$("#checklist option[value="+ idChecklist +"]").text()+'</a></td><td class="col1A '+exportar+'" style="vertical-align: middle;">'+actividad+'</td><td class="col1 '+exportar+'" style="vertical-align: middle;" title="'+idSucursal+'">'+nombre+'</td><td class="col2 '+exportar+'" style="vertical-align: middle;" title="'+idGrupo+'">'+nombreGrupo+'</td><td class="col7 '+exportar+'" style="vertical-align: middle;">'+nombreUsuario+'</td>'+precargaCols+'<td class="col4 '+exportar+'" style="vertical-align: middle;">'+fecha+'</td><td class="col5 '+exportar+'" style="vertical-align: middle;">'+fechaAsignacion+'</td><td class="col6 '+exportar+'" style="vertical-align: middle;">'+fechaAtencion+'</td><td class="'+exportar+'" style="vertical-align: middle;">'+tiempo+'</td><td class="col0 '+exportar+'" style="vertical-align: middle;width: 1%;white-space: nowrap;">'+clicUpd+'</td><td class="opciones opTick" style="vertical-align: middle;"><button class="btnMas" title="Historial" data-toggle="modal" data-target="#ticketsHistorico" onclick="getHistorico(\''+idRow+'\')" style=""><i class="fas fa-plus"></i></button>'+btnMas+'</td></tr>';
 return interfaz;
}
